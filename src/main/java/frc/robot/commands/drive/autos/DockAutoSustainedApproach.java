...
public class DockAutoSustainedApproach extends CommandBase {
  private SwerveSubsystem swerveSubsystem;
  private double angleRange;
  private boolean didReachDock;
  private boolean isFinished;
  private double reachedDockTimestamp;
  ...
  public DockAutoSustainedApproach(SwerveSubsystem swerveSubsystem, double angleRange, double MAX_POWER_METERS,
      double MAX_ANGLE,
  ...
  public void execute() {
    ...
    double timerThreshold = .4;
    // 14 might be too high.

    if (currentPitch < approachPitchThreshold) {
      reachedDockTimestamp = 0;
    }

    if (currentPitch >= approachPitchThreshold && !didReachDock) {
      if (reachedDockTimestamp == 0) {
        reachedDockTimestamp = Timer.getFPGATimestamp();
      }

      if ((reachedDockTimestamp - Timer.getFPGATimestamp()) > timerThreshold) {
        didReachDock = true;
        watchdog.reset();
      }
  ...
  public boolean isFinished() {
    ...
    if (didFinish) {
      Telemetry.logData("---- Dock Auto ----; didFinish", didFinish, DockAutoSustainedApproach.class);
    }

    if (watchdogExpired) {
      Telemetry.logData("---- Dock Auto ----; watchdog expired", watchdogExpired, DockAutoSustainedApproach.class);
    }
